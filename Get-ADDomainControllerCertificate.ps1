...
function Get-ADLDAPUnsecureConnection
{
    ...
    PROCESS
    {
        ...
        {
            $Events = Get-WinEvent -FilterHashtable @{LogName="Directory Service";ID='2889';IPAddress='172.21.24.20';StartTime="$((Get-Date).AddDays(-$LastDays))"} -ComputerName $Computer -ErrorAction SilentlyContinue -MaxEvents 100
...
####################################################################################
#Get-ADDomainControllerDiagnostics 
Get-ADLDAPUnsecureConnection -LastDays 100 | Out-GridView